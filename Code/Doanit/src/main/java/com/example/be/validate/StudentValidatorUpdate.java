...
public class StudentValidatorUpdate {
    ...
    public Map<String, String> validate(CreateUpdateStudentDTO createUpdateStudentDTO) {
        ...
        IStudentEditDTO studentEditDTO = studentRepository.findStudentByStudentId(createUpdateStudentDTO.getStudentId());
        List<IStudentEditDTO> students = studentRepository.findStudentByPhoneUpdate(studentEditDTO.getPhone(), createUpdateStudentDTO.getPhone());
        if (!students.isEmpty()) {
            errors.put("errorPhoneDuplicate", "Số điện thoại đã tồn tại, vui lòng nhập số điện thoại khác");
        }

        students = studentRepository.findStudentByEmailUpdate(studentEditDTO.getEmail(), createUpdateStudentDTO.getEmail());
        if (!students.isEmpty()) {
